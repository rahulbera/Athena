...
typedef enum {
  ...
  DTLB,
  DTLB_MSHR,
  STLB, // STLB does not have MSHR. Anything that misses STLB directly
        // emiulates PTW
  PTW,
  L1I,
  L1I_RQ,
...
public:
  ...
  uint8_t instruction, is_data, fill_l1i, fill_l1d, tlb_access, scheduled, translated, fetched, prefetched, drc_tag_read;

  int fill_level, pf_origin_level, rob_signal, rob_index, rob_position, producer, delta, depth, signature, confidence;
  ...
  uint64_t address, full_addr, instruction_pa, data_pa, data, instr_id, ip, event_cycle, cycle_enqueued, enque_cycle[NUM_MODULE_TYPES][NUM_QUEUE_TYPES],
  ...
  uint8_t pf_went_offchip_pred;    // predicted to go off-chip for prefetch
  uint8_t pf_went_offchip;         // 1 => this prefetch went to DRAM
  float perceptron_weights_sum;    // perceptron weight sum for TLP
  ocp_base_feature_t *ocp_feature; // TLP feature information
  ...
  PACKET() {
    ...
    // TLP-specific initialization
    went_offchip = 0;
    pf_went_offchip_pred = 0;
    pf_went_offchip = 0;
    perceptron_weights_sum = 0.0;
    ocp_feature = NULL;
  };
...
public:
  ...
  uint32_t cpu, occupancy, num_returned, next_fill_index, next_schedule_index, next_process_index;
...
public:
  ...
  inline bool is_empty() {
    return entry[head].cpu == NUM_CPUS ? true : false;
  }
  inline PACKET &get_entry(int32_t index) {
    return entry[index];
  }
...
public:
  ...
  int32_t head;
  deque<PACKET>::iterator head_iterator; // iterator to head; used in
                                         // remove_queue()
...
public:
  const string NAME;
  const uint32_t SIZE;
  uint32_t cpu, head, tail, occupancy, next_schedule;
  uint64_t event_cycle, fetch_event_cycle, schedule_event_cycle, execute_event_cycle, lsq_event_cycle, retire_event_cycle;
...
class LSQ_ENTRY {
public:
  uint64_t instr_id, producer_id, virtual_address, physical_address, ip, event_cycle;
