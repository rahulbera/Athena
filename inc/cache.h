...
public:
  ...
  uint64_t total_miss_latency, total_load_miss_latency;
  ...
  void add_mshr(PACKET *packet), update_fill_cycle(), llc_initialize_replacement(), llc_replacement_print_config(),
      ...
      lru_update(uint32_t set, uint32_t way), fill_cache(uint32_t set, uint32_t way, PACKET *packet), replacement_final_stats(), llc_replacement_final_stats(),
  ...
  void l1d_prefetcher_print_config(), l2c_prefetcher_print_config(), llc_prefetcher_print_config();
  ...
  void broadcast_bw(uint8_t bw_level), l1d_prefetcher_broadcast_bw(uint8_t bw_level), l2c_prefetcher_broadcast_bw(uint8_t bw_level),
      llc_prefetcher_broadcast_bw(uint8_t bw_level);

  void broadcast_ipc(uint8_t ipc), l1d_prefetcher_broadcast_ipc(uint8_t ipc), l2c_prefetcher_broadcast_ipc(uint8_t ipc),
      llc_prefetcher_broadcast_ipc(uint8_t ipc);

  void handle_prefetch_feedback();

  void broadcast_acc(uint32_t acc_level), l1d_prefetcher_broadcast_acc(uint32_t bw_level), l2c_prefetcher_broadcast_acc(uint32_t bw_level),
      llc_prefetcher_broadcast_acc(uint32_t bw_level);

  // ZLANG: pollution
  uint64_t get_prefetch_pollution_count() const {
    return prefetch_pollution_count;
  }

  void reset_prefetch_pollution_count() {
    prefetch_pollution_count = 0;
  }
};
