...
void print_core_roi_stats(uint32_t cpu) {
  ...
  std::sort(pairs.begin(), pairs.end(), [](const std::pair<uint64_t, load_per_ip_info_t> &p1, const std::pair<uint64_t, load_per_ip_info_t> &p2) {
  ...
  cout << "pc|total_loads|loads_went_offchip|loads_went_offchip_pos[0]|loads_"
          "went_offchip_pos[1]|..."
       << endl;
  uint64_t load_ips = 0, frontal_bias_load_ips[5] = {0}, dorsal_bias_load_ips[5] = {0};
  float bias_values[5] = {0.9, 0.8, 0.7, 0.6, 0.5};
  uint64_t frontal_bias_load_ips_total_loads[5] = {0}, frontal_bias_load_ips_off_chip_loads[5] = {0}, dorsal_bias_load_ips_total_loads[5] = {0},
           dorsal_bias_load_ips_off_chip_loads[5] = {0};
  for (auto it = pairs.begin(); it != pairs.end(); ++it) {
    cout << setw(10) << hex << it->first << dec << "|" << setw(10) << it->second.total_loads << "|" << setw(10) << it->second.loads_went_offchip << "|";
    ...
    for (uint32_t i = 0; i < 5; ++i) {
      ...
      if ((float)frontal_loads_went_offchip / it->second.loads_went_offchip >= bias_values[i]) {
        ...
        // keep track of total number of loads and off-chip loads generated by
        // all frontal-biased IPs
  ...
  std::sort(pairs2.begin(), pairs2.end(), [](const std::pair<uint64_t, load_per_ip_info_t> &p1, const std::pair<uint64_t, load_per_ip_info_t> &p2) {
  ...
  cout << "Core_" << cpu << "_DDRP_total " << ooo_cpu[cpu].stats.ddrp.total << endl
       ...
       << "Core_" << cpu << "_DDRP_issued_after_prefetch " << ooo_cpu[cpu].stats.ddrp.issued[2] << endl // only for TLP
  ...
  // OOGWAY STATS
  if (ooo_cpu[cpu].oogway) {
    ooo_cpu[cpu].oogway->dump_stats();
  }
}
...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
  ...
  cout << "Core_" << cpu << "_" << cache->NAME << "_total_access " << TOTAL_ACCESS << endl
       ...
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_all_reuse_avg "
       << (float)cache->stats.eviction.all_reuse_total / cache->stats.eviction.atleast_one_reuse << endl;
  ...
  for (uint32_t type = LOAD; type < NUM_TYPES; ++type) {
    cout << "Core_" << cpu << "_" << cache->NAME << "_eviction_" << type_names[type] << "_reuse_total " << cache->stats.eviction.cat_reuse_total[type] << endl
         ...
         << "Core_" << cpu << "_" << cache->NAME << "_eviction_" << type_names[type] << "_reuse_avg "
         << (float)cache->stats.eviction.cat_reuse_total[type] / cache->stats.eviction.atleast_one_reuse_cat[type] << endl;
  }
  ...
  {
    if (pairs[index].first <= 0) {
      dep_bucket[0] += pairs[index].second;
    } else if (pairs[index].first <= 2) {
      dep_bucket[1] += pairs[index].second;
    } else if (pairs[index].first <= 5) {
      dep_bucket[2] += pairs[index].second;
    } else if (pairs[index].first <= 9) {
      dep_bucket[3] += pairs[index].second;
    } else {
      dep_bucket[4] += pairs[index].second;
    }
  }
  ...
  if (cache->cache_type == IS_LLC) {
    ...
    for (uint32_t index = 0; index < ROB_SIZE; ++index) {
      if (!knob::rob_partition_boundaries.empty() && (int32_t)index == knob::rob_partition_boundaries[part_id]) {
        cout << "Core_" << cpu << "_" << cache->NAME << "_missing_load_rob_pos_hist_part_" << part_id << " " << sum << endl;
        cout << "Core_" << cpu << "_" << cache->NAME << "_missing_load_rob_pos_hist_part_" << part_id << "_perc " << 100 * (float)sum / total_load_miss << endl;
        sum = 0;
        part_id++;
      }
...
void print_branch_stats() {
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    cout << "Core_" << i << "_branch_pred_accuracy " << (100.0 * (ooo_cpu[i].num_branch - ooo_cpu[i].branch_mispredictions)) / ooo_cpu[i].num_branch << endl
         << "Core_" << i << "_branch_pred_mpki " << (1000.0 * ooo_cpu[i].branch_mispredictions) / (ooo_cpu[i].num_retired - ooo_cpu[i].warmup_instructions)
         << endl
         << "Core_" << i << "_avg_ROB_occupancy_at_mispred " << (1.0 * ooo_cpu[i].total_rob_occupancy_at_branch_mispredict) / ooo_cpu[i].branch_mispredictions
         << endl
         << endl
    ...
    //     cout << (100.0*(ooo_cpu[i].num_branch -
    //     ooo_cpu[i].branch_mispredictions)) / ooo_cpu[i].num_branch; cout <<
    //     "% MPKI: " <<
    //     (1000.0*ooo_cpu[i].branch_mispredictions)/(ooo_cpu[i].num_retired -
    //     ooo_cpu[i].warmup_instructions);
    ...
    // ooo_cpu[i].begin_sim_instr) << "%" << endl; cout <<
    // "BRANCH_INDIRECT_CALL: " << ooo_cpu[i].total_branch_types[5] << " " <<
...
void print_addr_translation_stats(uint32_t cpu) {
  cout << "Core_" << cpu << "_major_fault " << major_fault[cpu] << endl << "Core_" << cpu << "_minor_fault " << minor_fault[cpu] << endl << endl;
}
...
void print_deadlock(uint32_t i) {
  ...
  for (uint32_t j = 0; j < LQ_SIZE; j++) {
    cout << "[LQ] entry: " << j << " instr_id: " << ooo_cpu[i].LQ.entry[j].instr_id << " address: " << hex << ooo_cpu[i].LQ.entry[j].physical_address << dec
         << " translated: " << +ooo_cpu[i].LQ.entry[j].translated << " fetched: " << +ooo_cpu[i].LQ.entry[i].fetched << endl;
  }

  // print SQ entry
  cout << endl << "Store Queue Entry" << endl;
  for (uint32_t j = 0; j < SQ_SIZE; j++) {
    cout << "[SQ] entry: " << j << " instr_id: " << ooo_cpu[i].SQ.entry[j].instr_id << " address: " << hex << ooo_cpu[i].SQ.entry[j].physical_address << dec
         << " translated: " << +ooo_cpu[i].SQ.entry[j].translated << " fetched: " << +ooo_cpu[i].SQ.entry[i].fetched << endl;
  }
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage, uint8_t is_code) {
  ...
  uint8_t swap = 0;
  uint64_t high_bit_mask = rotr64(cpu, lg2(NUM_CPUS)), unique_va = va | high_bit_mask;
  ...
  if (cl_check == unique_cl[cpu].end()) { // we've never seen this cache line
                                          // before
  ...
  if (pr == page_table.end()) { // no VA => PA translation found
    ...
    if (allocated_pages >= DRAM_PAGES) { // not enough memory
      ...
      while (1) {                                       // try to find an empty physical page number
        ppage_check = inverse_table.find(random_ppage); // check if this page
                                                        // can be allocated
        if (ppage_check != inverse_table.end()) {       // random_ppage is not
                                                        // available
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < NUM_CPUS; i++) {
    ...
    // ooo_cpu[i].DTLB.MAX_READ = (2 > MAX_READ_PER_CYCLE) ? MAX_READ_PER_CYCLE
    // : 2;
  ...
  while (run_simulation) {
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
      ...
      // OG record event
      ooo_cpu[i].og_record_event(CYCLE, 1, i);
      ...
      if (stall_cycle[i] <= current_core_cycle[i]) {
        ...
        if ((ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].executed == COMPLETED) &&
            (ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].event_cycle <= current_core_cycle[i])) {
      ...
      if (all_warmup_complete == NUM_CPUS) // this part is called only once when
                                           // all cores are warmed up
      {
        all_warmup_complete++;
        finish_warmup();
      }
    ...
    if (knob::measure_dram_bw && uncore.cycle >= uncore.DRAM.next_bw_measure_cycle) {
      ...
      uncore.DRAM.og_bw = min(100u, quartile);
      if (quartile <= 25) {
        uncore.DRAM.bw = 0;
      } else if (quartile <= 50) {
        uncore.DRAM.bw = 1;
      ...
      MYLOG("cycle %lu rq_enqueue_count %lu last_enqueue_count %lu "
            "epoch_enqueue_count %lu QUARTILE %u",
            uncore.cycle, uncore.DRAM.rq_enqueue_count, uncore.DRAM.last_enqueue_count, uncore.DRAM.epoch_enqueue_count, uncore.DRAM.bw);
  ...
  if (NUM_CPUS > 1) {
    //         cout << endl << "Total Simulation Statistics (not including
    //         warmup)" << endl; for (uint32_t i=0; i<NUM_CPUS; i++)
    //         {
    //             cout << "Core_" << i << "_cumulative_IPC " << (float)
    //             (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) /
    //             (current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle) << endl
    //                 << "Core_" << i << "_total_instructions " <<
    //                 ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr <<
    //                 endl
    //                 << "Core_" << i << "_cycles " << current_core_cycle[i] -
    //                 ooo_cpu[i].begin_sim_cycle << endl
    //                 << endl;
