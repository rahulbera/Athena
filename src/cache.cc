...
void CACHE::create_rq() {
  ...
  if (cache_type == IS_ITLB && knob::enable_itlb_priority_rq) {
    priority_rq = true;
    priority_type = knob::itlb_priority_rq_priority_type;
  } else if (cache_type == IS_DTLB && knob::enable_dtlb_priority_rq) {
    priority_rq = true;
    priority_type = knob::dtlb_priority_rq_priority_type;
  } else if (cache_type == IS_STLB && knob::enable_stlb_priority_rq) {
    priority_rq = true;
    priority_type = knob::stlb_priority_rq_priority_type;
  } else if (cache_type == IS_L1I && knob::enable_l1i_priority_rq) {
    priority_rq = true;
    priority_type = knob::l1i_priority_rq_priority_type;
  } else if (cache_type == IS_L1D && knob::enable_l1d_priority_rq) {
    priority_rq = true;
    priority_type = knob::l1d_priority_rq_priority_type;
  } else if (cache_type == IS_L2C && knob::enable_l2c_priority_rq) {
    priority_rq = true;
    priority_type = knob::l2c_priority_rq_priority_type;
  } else if (cache_type == IS_LLC && knob::enable_llc_priority_rq) {
    priority_rq = true;
    priority_type = knob::llc_priority_rq_priority_type;
  }
...
void CACHE::handle_fill() {
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (cache_type == IS_LLC) {
      way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr,
                            MSHR.entry[mshr_index].type);
    } else {
      way = find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr,
                        MSHR.entry[mshr_index].type);
    }
                                                    ...
                                                    // does not fill the LLC
    {
      ...
      if (warmup_complete[fill_cpu] && (MSHR.entry[mshr_index].cycle_enqueued != 0)) {
        ...
        if (MSHR.entry[mshr_index].is_data && MSHR.entry[mshr_index].type == LOAD) {
          total_load_miss_latency += current_miss_latency;
          if (cache_type == IS_LLC) {
            ooo_cpu[fill_cpu].og_record_event(LLC_LOAD_MISS_LAT, current_miss_latency, fill_cpu);
          }
        }
      }
    ...
    if (do_fill) {
      ...
      if (cache_type == IS_L1I) {
        l1i_prefetcher_cache_fill(fill_cpu, ((MSHR.entry[mshr_index].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE, set, way,
                                  (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, ((block[set][way].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE);
      }
      if (cache_type == IS_L1D) {
        l1d_prefetcher_cache_fill(MSHR.entry[mshr_index].full_addr, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0,
                                  block[set][way].address << LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].pf_metadata);
      }
      if (cache_type == IS_L2C) {
        MSHR.entry[mshr_index].pf_metadata =
            l2c_prefetcher_cache_fill(MSHR.entry[mshr_index].address << LOG2_BLOCK_SIZE, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0,
                                      block[set][way].address << LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].pf_metadata);
      }
      if (cache_type == IS_LLC) {
        cpu = fill_cpu;
        MSHR.entry[mshr_index].pf_metadata =
            llc_prefetcher_cache_fill(MSHR.entry[mshr_index].address << LOG2_BLOCK_SIZE, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0,
                                      block[set][way].address << LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].pf_metadata);
        cpu = 0;
      }
      ...
      // update replacement policy
      if (cache_type == IS_LLC) {
        llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr,
                                     MSHR.entry[mshr_index].type, 0);
      } else {
        update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr,
                                 MSHR.entry[mshr_index].type, 0);
      }
      ...
      // // @RBERA: if this is a load fill in LLC, then monitor the position of
      // the load in ROB if(cache_type == IS_LLC && MSHR.entry[mshr_index].type
      // == LOAD)
      // {

      //     if(MSHR.entry[mshr_index].rob_position < 0 &&
      //     MSHR.entry[mshr_index].rob_position >= ROB_SIZE)
      //     {
      //         cout << "invalid ROB position: index: " << mshr_index << " pos:
      //         " << MSHR.entry[mshr_index].rob_position << endl; assert(0);
      //     }
      //     //
      ...
      // ZLANG: Update eviction status if the block is evicted due to a prefetch
      // request
      if (block[set][way].valid && /*!block[set][way].prefetch &&*/
          MSHR.entry[mshr_index].type == PREFETCH) {
        prefetch_eviction_map[block[set][way].address << LOG2_BLOCK_SIZE] = true;
      }
      ...
      if (warmup_complete[fill_cpu] && (MSHR.entry[mshr_index].cycle_enqueued != 0)) {
        ...
        MSHR.entry[mshr_index].cycle_enqueued << " = " << current_miss_latency
        << " MSHR index: " << mshr_index << endl;
        }
        */
        total_miss_latency += current_miss_latency;
        if (MSHR.entry[mshr_index].is_data && MSHR.entry[mshr_index].type == LOAD) {
...
void CACHE::handle_writeback() {
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
    ...
    {
      WQ.entry[index].hit_where = assign_hit_where(cache_type,
                                                   0); // writeback hit
    ...
    } else // writeback miss (or RFO miss for L1D)
    {
      ...
      if (cache_type == IS_L1D) // RFO miss
      {
        ...
        if (do_fill) {
          ...
          if (cache_type == IS_L1I) {
            l1i_prefetcher_cache_fill(writeback_cpu, ((WQ.entry[index].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE, set, way, 0,
          ...
          // update replacement policy
          if (cache_type == IS_LLC) {
            llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr,
                                         WQ.entry[index].type, 0);
          } else {
            update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type,
                                     0);
          }
    ...
    if ((RQ->peek().event_cycle <= current_core_cycle[read_cpu]) && (RQ->occupancy > 0)) {
      ...
      if (way >= 0) // read hit
      {
        ...
        if (knob::track_load_hit_dependency_in_cache) {
          ...
          for (uint32_t i = 0; i < DEP_INSTR_TYPES; ++i) {
            block[set][way].cat_dependents[i] = cat_dependents[i];
          }
          // if(warmup_complete[read_cpu] && cache_type == IS_LLC &&
          // rq_entry.type == LOAD && dependents == 0)
          // {
      ...
      } else // read miss
      {
          ...
          if (cache_type == IS_LLC) {
            ...
            if (lower_level) {
              if (lower_level->get_occupancy(1, rq_entry.address) == lower_level->get_size(1, rq_entry.address)) {
                ...
                lower_level->add_rq(&rq_entry);
                if (cache_type == IS_L1D && rq_entry.is_data && rq_entry.type == LOAD) {
                  ooo_cpu[read_cpu].og_record_event(L1D_LOAD_MISS, 1, read_cpu);
                }
              }
          ...
          } else if (mshr_index != -1) // already in-flight miss
          {
            ...
            if (rq_entry.type == RFO) {
              ...
              if (rq_entry.instruction) {
                ...
                MSHR.entry[mshr_index].instruction = 1; // add as instruction
                                                        // type
        ...
        if (miss_handled) {
          ...
          if (rq_entry.type == LOAD) {
            ...
            // RBERA: if TLP is enabled and the threshold is between tau_1 and tau_2, issue ddrp request now.
            if (cache_type == IS_L1D && rq_entry.is_data && rq_entry.type == LOAD && rq_entry.lq_index < LQ_SIZE) {
              // Check if TLP is enabled and access the LQ entry to get TLP prediction weight
              if (knob::offchip_pred_type == "tlp" && ooo_cpu[rq_entry.cpu].offchip_pred) {
                LSQ_ENTRY &lq_entry = ooo_cpu[rq_entry.cpu].LQ.entry[rq_entry.lq_index];
                if (lq_entry.ocp_feature) {
                  tlp_feature_t *feature = (tlp_feature_t *)lq_entry.ocp_feature;
                  float tlp_weight = feature->perc_weight_sum;

                  // Check if the weight is between tau_1 and tau_2 thresholds
                  if (tlp_weight <= knob::tlp_tau_1 && tlp_weight >= knob::tlp_tau_2) {
                    lq_entry.went_offchip_pred = 1;
                    ooo_cpu[rq_entry.cpu].issue_ddrp_request(rq_entry.lq_index, 2);
                  }
                }
              }
            }
...
void CACHE::handle_prefetch() {
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
      ...
      {
        PQ.entry[index].hit_where = assign_hit_where(cache_type,
                                                     0); // prefetch hit
        ...
        if (PQ.entry[index].pf_origin_level < fill_level) {
          if (cache_type == IS_L1D) {
            ...
            PQ.entry[index].pf_metadata =
                l2c_prefetcher_operate(block[set][way].address << LOG2_BLOCK_SIZE, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].pf_metadata);
          } else if (cache_type == IS_LLC) {
            cpu = prefetch_cpu;
            PQ.entry[index].pf_metadata =
                llc_prefetcher_operate(block[set][way].address << LOG2_BLOCK_SIZE, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].pf_metadata);
            cpu = 0;
          }
        }
        ...
        // TLP: train SLP for prefetch hits (prefetch did not go off-chip)
        if (knob::offchip_pred_type == "tlp" && ooo_cpu[PQ.entry[index].cpu].offchip_pred && PQ.entry[index].pf_origin_level == FILL_L1) {
          PQ.entry[index].pf_went_offchip = 0; // mark that this prefetch did not go off-chip
          ooo_cpu[PQ.entry[index].cpu].offchip_pred->train_on_prefetch(PQ.entry[index]);
        }
      ...
      } else // prefetch miss
      {
        ...
        } else if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) // this is a new miss
        {
          DP(if (warmup_complete[PQ.entry[index].cpu]) {
            cout << "[" << NAME << "_PQ] " << __func__ << " want to add instr_id: " << PQ.entry[index].instr_id << " address: " << hex
                 << PQ.entry[index].address;
            cout << " full_addr: " << PQ.entry[index].full_addr << dec;
            cout << " occupancy: " << lower_level->get_occupancy(3, PQ.entry[index].address) << " SIZE: " << lower_level->get_size(3, PQ.entry[index].address)
                 << endl;
          });

          // first check if the lower level PQ (or RQ, in case of LLC) is full
          // or not. this is possible since multiple prefetchers can exist at
          // each level of caches
          if (lower_level) {
            if (cache_type == IS_LLC) {
              if (lower_level->get_occupancy(1, PQ.entry[index].address) == lower_level->get_size(1, PQ.entry[index].address)) {
                ...
                if (PQ.entry[index].pf_origin_level < fill_level) {
                  if (cache_type == IS_LLC) {
                    cpu = prefetch_cpu;
                    PQ.entry[index].pf_metadata =
                        llc_prefetcher_operate(PQ.entry[index].address << LOG2_BLOCK_SIZE, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].pf_metadata);
                    cpu = 0;
                  }
                }
                ...
                // TLP: notify SLP that this prefetch went off-chip
                if (knob::offchip_pred_type == "tlp" && ooo_cpu[PQ.entry[index].cpu].offchip_pred && PQ.entry[index].pf_origin_level == FILL_L1) {
                  PQ.entry[index].pf_went_offchip = 1; // mark that this prefetch went off-chip
                  ooo_cpu[PQ.entry[index].cpu].offchip_pred->train_on_prefetch(PQ.entry[index]);
                }
              }
            } else {
              if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address)) {
                ...
                if (PQ.entry[index].pf_origin_level < fill_level) {
                  ...
                  if (cache_type == IS_L2C) {
                    PQ.entry[index].pf_metadata =
                        l2c_prefetcher_operate(PQ.entry[index].address << LOG2_BLOCK_SIZE, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].pf_metadata);
                  }
                }

                //====== TLP: Prefetch filtering for L1D ======//
                bool should_filter_prefetch = false;

                // Only apply prefetch filtering to L1D prefetches that originate from L1D prefetcher
                if (cache_type == IS_L1D && knob::offchip_pred_type == "tlp" && ooo_cpu[cpu].offchip_pred && PQ.entry[index].pf_origin_level == fill_level) {
                  // Get TLP prediction for prefetch filteringx
                  should_filter_prefetch = PQ.entry[index].pf_went_offchip_pred;
                }

                // Only add to MSHR and send to lower level if not filtered
                if (!should_filter_prefetch) {
                  if (PQ.entry[index].fill_level <= fill_level) {
                    add_mshr(&PQ.entry[index]);
                  }
                  lower_level->add_pq(&PQ.entry[index]); // add it to the PQ of lower cache
                } else if (should_filter_prefetch) {
                  // Increment dropped prefetch counter for filtered prefetches
                  pf_dropped++;
                  // Do not send filtered prefetches to lower level cache
                }
              }
            }
          }
...
int CACHE::check_hit(PACKET *packet) {
  ...
  // perfect cache
  if ((cache_type == IS_L1D && knob::l1d_perfect) || (cache_type == IS_L2C && knob::l2c_perfect) || (cache_type == IS_LLC && knob::llc_perfect)) {
    match_way = 0;
    return match_way;
  }
  ...
  // ZLANG
  if (match_way == -1) {
    auto it = prefetch_eviction_map.find(packet->address << LOG2_BLOCK_SIZE);
    if (it != prefetch_eviction_map.end() && it->second) {
      prefetch_pollution_count++;
      prefetch_eviction_map.erase(it);
    }
  }
  ...
  if (index != -1) {
    ...
    if (packet->instruction) {
      ...
      if (packet->type == RFO) {
        ...
        DP(if (warmup_complete[packet->cpu]) {
          cout << "[RFO merging in RQ] cache_type " << cache_type << " incoming_type " << packet->type << " exsisting_type " << rq_entry.type
               << " incoming_ROB_index " << packet->rob_index << " exsisting_ROB_index " << rq_entry.rob_index << " incoming_ROB_pos " << packet->rob_position
               << " exsisting_ROB_pos " << rq_entry.rob_position << endl;
        });
      } else {
        ...
        DP(if (warmup_complete[packet->cpu]) {
          cout << "[LOAD merging in RQ] cache_type " << cache_type << " incoming_type " << packet->type << " exsisting_type " << rq_entry.type
               << " incoming_ROB_index " << packet->rob_index << " exsisting_ROB_index " << rq_entry.rob_index << " incoming_ROB_pos " << packet->rob_position
               << " exsisting_ROB_pos " << rq_entry.rob_position << endl;
        });
...
int CACHE::add_wq(PACKET *packet) {
  ...
  // {
  //     cerr << "[" << NAME << "_ERROR] " << __func__ << " is not empty index:
  //     " << index; cerr << " address: " << hex << WQ.entry[index].address;
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata) {
  ...
  if (PQ.occupancy < PQ.SIZE) {
    if ((base_addr >> LOG2_PAGE_SIZE) == (pf_addr >> LOG2_PAGE_SIZE)) {
      ...
      // TLP: Get the FLP prediction of the triggering load
      // Find the triggering load in the load queue and get its FLP prediction
      bool triggering_load_flp_pred = false;
      if (knob::offchip_pred_type == "tlp" && ooo_cpu[cpu].offchip_pred && cache_type == IS_L1D) {
        // Search for the triggering load in the load queue
        for (uint32_t lq_idx = 0; lq_idx < ooo_cpu[cpu].LQ.SIZE; lq_idx++) {
          // rbera: still this address comparison logic is hacky
          if (ooo_cpu[cpu].LQ.entry[lq_idx].ip == ip && (ooo_cpu[cpu].LQ.entry[lq_idx].physical_address >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE == base_addr) {
            triggering_load_flp_pred = ooo_cpu[cpu].LQ.entry[lq_idx].went_offchip_pred;
            break;
          }
        }
      }
      pf_packet.went_offchip_pred = triggering_load_flp_pred;

      // TLP: predict whether this prefetch will go off-chip here itself using SLP
      if (cache_type == IS_L1D && knob::offchip_pred_type == "tlp" && ooo_cpu[cpu].offchip_pred) {
        pf_packet.pf_went_offchip_pred = ooo_cpu[cpu].offchip_pred->predict_on_prefetch(pf_packet);
      }
      ...
      // ZLANG: Reset eviction status when a prefetch request is serviced
      auto it = prefetch_eviction_map.find(pf_addr);
      if (it != prefetch_eviction_map.end()) {
        it->second = false;
      }
...
int CACHE::add_pq(PACKET *packet) {
  ...
  int wq_index = WQ.check_queue(packet);
  if (wq_index != -1) {
    // prefetch hitting in WQ
    packet->hit_where = assign_hit_where(cache_type, 2);
  ...
  //         cerr << "[" << NAME << "_ERROR] " << __func__ << " is not empty
  //         index: " << index; cerr << " address: " << hex <<
  //         PQ.entry[index].address; cerr << " full_addr: " <<
  //         PQ.entry[index].full_addr << dec << endl; assert(0);
  //     }
  // #endif
...
void CACHE::update_fill_cycle() {
  ...
  if (min_index < MSHR.SIZE) {
    DP(if (warmup_complete[MSHR.entry[min_index].cpu]) {
      ...
      cout << " event: " << MSHR.entry[min_index].event_cycle << " current: " << current_core_cycle[MSHR.entry[min_index].cpu]
...
int CACHE::check_mshr(PACKET *packet) {
  ...
  // if(instruction_and_data_collision) // remove instruction-and-data collision
  // safeguard
  //   {
  // return -2;
  //   }
...
void CACHE::track_stats_from_victim(uint32_t set, uint32_t way) {
  ...
  // keep count of blocks that have seen at least one reuse of specific access
  // type
  ...
  for (uint32_t index = 0; index < NUM_TYPES; ++index) {
    reuse += block[set][way].reuse[index];
  }
  stats.eviction.all_reuse_total += reuse;
  if (reuse >= stats.eviction.all_reuse_max) {
    stats.eviction.all_reuse_max = reuse;
  }
  if (reuse <= stats.eviction.all_reuse_min) {
    stats.eviction.all_reuse_min = reuse;
  }
  ...
  for (uint32_t type = LOAD; type < NUM_TYPES; ++type) {
    ...
    if (reuse >= stats.eviction.cat_reuse_max[type]) {
      stats.eviction.cat_reuse_max[type] = reuse;
    }
    if (reuse <= stats.eviction.cat_reuse_min[type]) {
      stats.eviction.cat_reuse_min[type] = reuse;
    }
  }

  // track cacheblocks that are reused by only frontal/dorsal loads
  if (block[set][way].reuse_frontal_dorsal[FRONTAL] > 0 && block[set][way].reuse_frontal_dorsal[DORSAL] == 0 &&
      block[set][way].reuse_frontal_dorsal[NONE] == 0) {
    stats.eviction.reuse_only_frontal++;
  } else if (block[set][way].reuse_frontal_dorsal[DORSAL] > 0 && block[set][way].reuse_frontal_dorsal[FRONTAL] == 0 &&
             block[set][way].reuse_frontal_dorsal[NONE] == 0) {
    stats.eviction.reuse_only_dorsal++;
  } else if (block[set][way].reuse_frontal_dorsal[NONE] > 0 && block[set][way].reuse_frontal_dorsal[FRONTAL] == 0 &&
  ...
  // track number of dependents for only those blocks that have seen at least
  // one load reuse
  if (knob::track_load_hit_dependency_in_cache && block[set][way].reuse[LOAD] > 0) {
    ...
    if (dep_all >= stats.eviction.dep_all_max) {
      stats.eviction.dep_all_max = dep_all;
    }
    if (dep_all <= stats.eviction.dep_all_min) {
      stats.eviction.dep_all_min = dep_all;
    }
    ...
    if (dep_branch_mispred >= stats.eviction.dep_branch_mispred_max) {
      stats.eviction.dep_branch_mispred_max = dep_branch_mispred;
    }
    if (dep_branch_mispred <= stats.eviction.dep_branch_mispred_min) {
      stats.eviction.dep_branch_mispred_min = dep_branch_mispred;
    }
    ...
    if (dep_branch >= stats.eviction.dep_branch_max) {
      stats.eviction.dep_branch_max = dep_branch;
    }
    if (dep_branch <= stats.eviction.dep_branch_min) {
      stats.eviction.dep_branch_min = dep_branch;
    }
    ...
    if (dep_load >= stats.eviction.dep_load_max) {
      stats.eviction.dep_load_max = dep_load;
    }
    if (dep_load <= stats.eviction.dep_load_min) {
      stats.eviction.dep_load_min = dep_load;
    }
  }
}
...
hit_where_t CACHE::assign_hit_where(uint8_t cache_type, uint32_t where_in_cache) {
  ...
  // 1 = hit in read queue (possible for L1D/L1I. Basically caches that are
  // core-faced) 2 = hit in WQ (possible for L1D/L1I). 3 = hit in MSHR

  if (cache_type == IS_ITLB) {
    // TODO: can ITLB have RQ/WQ hits too?
    if (where_in_cache == 0) {
      return hit_where_t::ITLB;
    } else if (where_in_cache == 3) {
      return hit_where_t::ITLB_MSHR;
    }
  } else if (cache_type == IS_DTLB) {
    ...
    if (where_in_cache == 0) {
      return hit_where_t::L1I;
    } else if (where_in_cache == 1) {
      return hit_where_t::L1I_RQ;
    } else if (where_in_cache == 2) {
      return hit_where_t::L1I_WQ;
    } else if (where_in_cache == 3) {
      return hit_where_t::L1I_MSHR;
    }
  } else if (cache_type == IS_L1D) {
    if (where_in_cache == 0) {
      return hit_where_t::L1D;
    } else if (where_in_cache == 1) {
      return hit_where_t::L1D_RQ;
    } else if (where_in_cache == 2) {
      return hit_where_t::L1D_WQ;
    } else if (where_in_cache == 3) {
      return hit_where_t::L1D_MSHR;
    }
  } else if (cache_type == IS_L2C) {
    if (where_in_cache == 0) {
      return hit_where_t::L2C;
    } else if (where_in_cache == 1) {
      return hit_where_t::L2C_RQ;
    } else if (where_in_cache == 2) {
      return hit_where_t::L2C_WQ;
    } else if (where_in_cache == 3) {
      return hit_where_t::L2C_MSHR;
    }
  } else if (cache_type == IS_LLC) {
    if (where_in_cache == 0) {
      return hit_where_t::LLC;
    } else if (where_in_cache == 1) {
      return hit_where_t::LLC_RQ;
    } else if (where_in_cache == 2) {
      return hit_where_t::LLC_WQ;
    } else if (where_in_cache == 3) {
      return hit_where_t::LLC_MSHR;
    }
  }
...
void CACHE::send_signal_to_core(uint32_t cpu, PACKET packet) {
  uint32_t lq_index = packet.lq_index;
  // uint32_t source_index = ooo_cpu[cpu].get_source_index_from_rob(rob_index,
  // lq_index);

#ifdef SANITY_CHECK
  uint32_t rob_index = packet.rob_index;
  if (ooo_cpu[cpu].LQ.entry[lq_index].rob_index != rob_index) {
    cout << "rob_index in LQ entry does not match with the rob_index from RQ entry" << " rob_index from LQ: " << ooo_cpu[cpu].LQ.entry[lq_index].rob_index
...
void CACHE::handle_prefetch_feedback() {
  ...
  if (knob::measure_cache_acc && cycle >= next_measure_cycle) {
    ...
    acc_level = (pref_acc / ((float)100 / CACHE_ACC_LEVELS)); // quantize into
                                                              // 8 buckets
...
void CACHE::broadcast_acc(uint32_t acc_level) {
  ...
  case IS_L1I:
    return;
  case IS_L1D:
    return l1d_prefetcher_broadcast_acc(acc_level);
  case IS_L2C:
    return l2c_prefetcher_broadcast_acc(acc_level);
  case IS_LLC:
    return llc_prefetcher_broadcast_acc(acc_level);
  }
}
